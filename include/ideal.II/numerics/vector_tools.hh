...
namespace VectorTools{
	 ...
	 */
	void
	extract_subvector_at_time_dof(const dealii::TrilinosWrappers::MPI::Vector& spacetime_vector,
	...
	template< int dim>
	void
	extract_subvector_at_time_point(slab::DoFHandler<dim>& dof_handler,
		...
		for (auto cell: dof_handler.temporal()->active_cell_iterators()){
			...
			dealii::FEValues<1> time_values(dof_handler.temporal()->get_fe(),
											time_qf,dealii::update_values);
			time_values.reinit(cell);
			unsigned int offset = cell->index()*dof_handler.dofs_per_cell_time()*n_dofs_space;
