...
namespace idealii{
  ...
  TimeIteratorCollection(){
	  ...
	  par_dist_tria.it_collection = std::vector<slab::parallel::distributed::TriaIterator<dim>*>();
	  par_dist_tria.obj_collection = std::vector<spacetime::parallel::distributed::Triangulation<dim>*>();
  ...
  void
  TimeIteratorCollection<dim>::
  add_iterator(slab::TriaIterator<dim>* it,spacetime::Triangulation<dim>* obj){
  ...
  void
  TimeIteratorCollection<dim>::
  add_iterator(slab::parallel::distributed::TriaIterator<dim>* it,
  ...
  void
  TimeIteratorCollection<dim>::
  add_iterator(slab::DoFHandlerIterator<dim>* it, spacetime::DoFHandler<dim>* obj){
  ...
  void
  TimeIteratorCollection<dim>::
  add_iterator(slab::VectorIterator<double>* it, spacetime::Vector<double>* obj){
  ...
  void
  TimeIteratorCollection<dim>::
  add_iterator(slab::TrilinosVectorIterator* it, spacetime::TrilinosVector* obj){
  ...
  increment(){
	  ...
	  for (unsigned int i = 0 ; i < par_dist_tria.it_collection.size() ; i++){
		  Assert(*par_dist_tria.it_collection[i]!=par_dist_tria.obj_collection[i]->end(),dealii::ExcIteratorPastEnd());
		  ++(*par_dist_tria.it_collection[i]);
	  }
#endif
	  for (unsigned int i = 0 ; i < dof.it_collection.size() ; i++){
		  Assert(*dof.it_collection[i]!=dof.obj_collection[i]->end(),dealii::ExcIteratorPastEnd());
		  ++(*dof.it_collection[i]);
	  }
	  ...
	  for (unsigned int i = 0 ; i < trilinos_vector.it_collection.size() ; i++){
		  Assert(*trilinos_vector.it_collection[i]!=trilinos_vector.obj_collection[i]->end(),dealii::ExcIteratorPastEnd());
  ...
  decrement(){
	  for (unsigned int i = 0 ; i < tria.it_collection.size() ; i++){
		  Assert(*tria.it_collection[i]!=std::prev(tria.obj_collection[i]->begin()),dealii::ExcIteratorPastEnd());
		  --(*tria.it_collection[i]);
	  }

#ifdef DEAL_II_WITH_MPI
	  for (unsigned int i = 0 ; i < par_dist_tria.it_collection.size() ; i++){
		  Assert(*par_dist_tria.it_collection[i]!=std::prev(par_dist_tria.obj_collection[i]->begin()),dealii::ExcIteratorPastEnd());
		  --(*par_dist_tria.it_collection[i]);
	  }
#endif
	  for (unsigned int i = 0 ; i < dof.it_collection.size() ; i++){
		  Assert(*dof.it_collection[i]!=std::prev(dof.obj_collection[i]->begin()),dealii::ExcIteratorPastEnd());
		  --(*dof.it_collection[i]);
	  }
	  for (unsigned int i = 0 ; i < vector_double.it_collection.size() ; i++){
		  Assert(*vector_double.it_collection[i]!=std::prev(vector_double.obj_collection[i]->begin()),dealii::ExcIteratorPastEnd());
		  --(*vector_double.it_collection[i]);
	  }
#ifdef DEAL_II_WITH_TRILINOS
#ifdef DEAL_II_WITH_MPI
	  for (unsigned int i = 0 ; i < trilinos_vector.it_collection.size() ; i++){
		  Assert(*trilinos_vector.it_collection[i]!=std::prev(trilinos_vector.obj_collection[i]->begin()),dealii::ExcIteratorPastEnd());
		  --(*trilinos_vector.it_collection[i]);
	  }
#endif
#endif
