...
namespace fixed{
	...
	void
	Triangulation<dim>::
	generate(std::shared_ptr<dealii::parallel::distributed::Triangulation<dim>> space_tria,
		...
		for (unsigned int i = 0; i < M ; i++ ){
			this->trias.push_back(idealii::slab::parallel::distributed::Triangulation<dim>(space_tria,t,t+k));
			t+=k;
		}
	}
