...
void Step2::setup_system_on_slab(){
	...
	std::cout << "Number of degrees of freedom: \n\t"
				  ...
				  << slab_its.dof->n_dofs_spacetime() << " (spacetime)"
				  << std::endl;
	if ( slab == 0){
	...
	idealii::slab::DoFTools::make_upwind_sparsity_pattern(*slab_its.dof,coupling_space,dsp,slab_constraints);
...
void Step2::assemble_system_on_slab(){
	idealii::spacetime::QGauss<2> quad(fe.spatial()->degree+3,fe.temporal()->degree+1);
...
void Step2::output_results_on_slab(){
	...
	for ( unsigned i = 0 ; i < n_dofs ; i++){
		...
		idealii::slab::VectorTools::extract_subvector_at_time_dof(*slab_its.solution, local_solution,i);
		...
		filename << "solution_dG(" << fe.temporal()->degree
				<< ")_t_" << slab*n_dofs+i << ".vtk";
