...
:
                ...
                pout ( std::cout ,
                       dealii::Utilities::MPI::this_mpi_process ( mpi_comm ) == 0 ), triangulation (), dof_handler (
                               &triangulation ), fe (
                                       std::make_shared < dealii::FE_Q < 2 >> ( spatial_degree ) ,
                                       temporal_degree ,
                                       idealii::spacetime::DG_FiniteElement < 2 > ::support_type::Legendre ), slab (
                                               0 ), exact_solution ()
{
}
...
void Step3::time_marching ()
{
    ...
    for ( ; !tic.at_end () ; tic.increment () )
    {
        ...
        idealii::slab::VectorTools::extract_subvector_at_time_point (
                *slab_its.dof , *slab_its.solution , slab_initial_value ,
...
void Step3::setup_system_on_slab ()
{
    ...
    idealii::slab::VectorTools::interpolate_boundary_values (
            space_locally_relevant_dofs , *slab_its.dof , 0 , zero ,
            slab_constraints );
                ...
                for ( unsigned int q = 0 ; q < n_quad_spacetime ; ++q )
                {
                            ...
                            ++i )
                    {
                                ...
                                ++j )
                        {
                            ...
                            cell_matrix ( i + n * dofs_per_spacetime_cell ,
                                          j + n * dofs_per_spacetime_cell ) +=
                                                  fe_values_spacetime.shape_space_grad ( i ,
                                                                                         q )
                                                                                         * fe_values_spacetime.shape_space_grad ( j ,
                                                                                                                                  q )
                                                                                                                                  * fe_values_spacetime.JxW ( q );
                        } //dofs j
