...
private:
	...
	void assemble_residual_on_slab();
	void assemble_system_on_slab();
	void solve_Newton_problem_on_slab();
...
void Step4::setup_system_on_slab(){
	...
	dealii::Table<2,dealii::DoFTools::Coupling> coupling_space(
		2+1, 2+1
	);
...
void Step4::assemble_system_on_slab(){
    ...
    std::vector<dealii::Vector<double>> old_solution_values(n_quad_space,
    														dealii::Vector<double>(3));

    std::vector<std::vector<dealii::Tensor<1,2> > >
    		old_solution_grad(n_quad_space,std::vector<dealii::Tensor<1,2>>(3));
	...
	if(cell_space ->is_locally_owned()){
		...
		fe_values_spacetime.spatial()->operator[](velocity).get_function_values(slab_initial_value, initial_values);
		...
		for (const auto &cell_time : slab_its.dof->temporal()->active_cell_iterators()){
			...
			idealii::slab::VectorTools::extract_subvector_at_time_point(
					*slab_its.dof,
					*slab_its.solution, solution_extract,
...
void Step4::assemble_residual_on_slab(){
	...
	if(cell_space ->is_locally_owned()){
		...
		for (const auto &cell_time : slab_its.dof->temporal()->active_cell_iterators()){
			...
			if( n == 0){
				fe_values_spacetime.spatial()->operator[](velocity).get_function_values(slab_initial_value, solution_minus);
			}
			for (unsigned int q = 0  ; q < n_quad_spacetime ; ++q){
...
//				// convection
			...
			if( n < N-1){
				idealii::slab::VectorTools::extract_subvector_at_time_point(
					...
					*slab_its.solution, solution_extract,
					cell_time->face(1)->center()[0]);
					fe_values_spacetime.spatial()->operator[](velocity).get_function_values(solution_extract, solution_minus);
			}
	} //cell time
			slab_zero_constraints->distribute_local_to_global(
